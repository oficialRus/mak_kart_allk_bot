...
func main() {
	...
	if token == "" {
		log.Fatal("BOT_TOKEN is not set. Проверьте .env и переменную BOT_TOKEN.")
	}

	miniappURL := strings.TrimSpace(os.Getenv("MINI_APP_URL"))
	if miniappURL == "" {
		miniappURL = "https://app.garmonia-mak.ru/"
	}
	...
	const adminTelegramID int64 = 7217012505
	...
	for update := range updates {
		...
		// Админ может загружать новые карты дня: фото + подпись /upload_card
		if update.Message.From != nil &&
			update.Message.From.ID == adminTelegramID &&
