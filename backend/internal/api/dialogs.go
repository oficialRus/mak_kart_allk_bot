...
func DialogSaveHandler(botToken string) http.HandlerFunc {
	return withSessionOrInitData(botToken, func(w http.ResponseWriter, r *http.Request, userID int64) {
		...
		if err := repository.SaveDialog(r.Context(), userID, req.Mode, req.Title, req.Messages); err != nil {
			log.Printf("api dialog-save: save failed for user_id=%d: %v", userID, err)
...
func DialogsListHandler(botToken string) http.HandlerFunc {
	return withSessionOrInitData(botToken, func(w http.ResponseWriter, r *http.Request, userID int64) {
		...
		dialogs, err := repository.ListDialogs(r.Context(), userID, req.Limit)
		if err != nil {
			log.Printf("api dialogs: list failed for user_id=%d: %v", userID, err)
...
func DialogGetHandler(botToken string) http.HandlerFunc {
	return withSessionOrInitData(botToken, func(w http.ResponseWriter, r *http.Request, userID int64) {
		...
		dialog, err := repository.GetDialog(r.Context(), userID, id)
		if err != nil {
			log.Printf("api dialog-get: get failed for user_id=%d id=%d: %v", userID, id, err)
