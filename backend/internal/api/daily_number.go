...
func DailyNumberHandler(botToken string) http.HandlerFunc {
	return withSessionOrInitData(botToken, func(w http.ResponseWriter, r *http.Request, userID int64) {
		...
		now := time.Now()
		ctx := r.Context()
		num, err := repository.GetOrCreateDailyNumber(ctx, userID, now)
		if err != nil {
			log.Printf("api daily-number: GetOrCreateDailyNumber failed for user_id=%d: %v", userID, err)
		...
		if err != nil && err != sql.ErrNoRows {
			log.Printf("api daily-number: select existing message failed for user_id=%d: %v", userID, err)
		}
				...
				if apiKey != "" {
					...
					if errText != "" {
						...
						if _, err := db.Pool.ExecContext(
							...
							log.Printf("api daily-number: failed to update message for user_id=%d: %v", userID, err)
						}
					}
				}
			}
