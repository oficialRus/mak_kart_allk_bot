...
func CardDayHandler(botToken string) http.HandlerFunc {
	return withSessionOrInitData(botToken, func(w http.ResponseWriter, r *http.Request, userID int64) {
		...
		ctx := r.Context()
		outcome, err := repository.GetOrAssignCardOfDay(ctx, userID, exclusionDays)
		if err != nil {
			log.Printf("api card-day: GetOrAssignCardOfDay failed for user %d: %v", userID, err)
		...
		if cached != "" {
			...
			if apiKey == "" {
				log.Printf("api card-day: OPENAI_API_KEY is empty, using fallback day_message (user %d)", userID)
			} else {
				msg = generateCardDayMessageText(ctx, apiKey, card.Title, card.Description)
				if msg == "" {
					log.Printf("api card-day: AI returned empty day_message, fallback (user %d)", userID)
				}
			}
			if msg == "" {
			...
			resp.DayMessage = msg
			if err := repository.SaveCardDayCachedMessage(ctx, userID, msg); err != nil {
				log.Printf("api card-day: SaveCardDayCachedMessage failed for user %d: %v", userID, err)
			}
		}
