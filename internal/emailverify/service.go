...
func (s *Service) SendCode(ctx context.Context, telegramID int64, emailRaw string) (*SendCodeResult, error) {
	if s.Secret == "" {
	...
	nTG, err := repository.CountSendsTelegramSince(ctx, telegramID, since)
	if err != nil {
		return nil, err
	}
	...
	lastTG, err := repository.LastSendTimeForTelegram(ctx, telegramID)
	if err != nil {
		return nil, err
	}
	...
	owner, taken, err := repository.VerifiedEmailOwner(ctx, emailRaw, telegramID)
	if err != nil {
		return nil, err
	}
	if taken && owner != 0 {
		// Не раскрываем, что email занят — для клиента как rate limit / общая ошибка.
		log.Printf("emailverify: send blocked, email already verified by another user telegram_id=%d", telegramID)
		return nil, ErrInvalidEmail
	}

	expires := now.Add(CodeTTL)

	if err := repository.InvalidatePendingForTelegram(ctx, telegramID); err != nil {
		return nil, err
	}
	...
	hash := HashOTP(s.Secret, emailRaw, telegramID, code)
	if _, err := repository.InsertEmailVerification(ctx, telegramID, emailRaw, emailRaw, hash, expires, DefaultMaxAttempts, now); err != nil {
		return nil, err
	}
	...
	if s.MailSender != nil {
		if err := s.MailSender.Send(ctx, []string{emailRaw}, emailMsg); err != nil {
			log.Printf("emailverify: smtp send failed user_id=%d: %v", telegramID, err)
		} else {
			smtpOK = true
		}
	}
	...
	emailOK := smtpOK
	tgOK := false
	if !s.TelegramDisabled && telegramID > 0 && strings.TrimSpace(s.BotToken) != "" {
		...
		if sendTG {
			...
			if err := telegramapi.SendMessage(ctx, s.BotToken, tgText, telegramID); err != nil {
				log.Printf("emailverify: telegram OTP user_id=%d: %v (пользователь должен написать боту /start)", telegramID, err)
			} else {
				tgOK = true
			}
		}
	...
	default:
		if s.StrictSMTP {
			log.Printf("emailverify: ни SMTP, ни Telegram; STRICT — отказ user_id=%d", telegramID)
			return nil, ErrSendFailed
		}
		log.Printf("WARNING emailverify: код только в логе (нет доставки). user_id=%d email=%s code=%s", telegramID, emailRaw, code)
		delivery = DeliveryLogOnly
	}

	log.Printf("emailverify: otp issued user_id=%d email=%s delivery=%s tg=%v smtp=%v", telegramID, emailRaw, delivery, tgOK, smtpOK)
...
func (s *Service) VerifyCode(ctx context.Context, telegramID int64, emailRaw, code string) error {
	if s.Secret == "" {
	...
	hash := HashOTP(s.Secret, emailNorm, telegramID, code)
	return repository.TryConsumeOTP(ctx, telegramID, emailNorm, hash, time.Now().UTC())
}
