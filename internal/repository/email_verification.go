...
func TryConsumeOTP(ctx context.Context, telegramID int64, emailNormalized, codeHashHex string, now time.Time) error {
	...
	FOR UPDATE;
	`
	err = tx.QueryRowContext(ctx, q, telegramID, emailNormalized).Scan(&id, &storedHash, &expiresAt, &attempts, &maxAttempts)
	...
	err = tx.QueryRowContext(ctx, `
		...
		WHERE lower(trim(email)) = $1
		  ...
		  AND telegram_id <> $2
		LIMIT 1;
	`, emailNormalized, telegramID).Scan(&otherOwner)
